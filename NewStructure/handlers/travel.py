...
class TravelHandler(InputSender):
    ...
    def move_to_position(self, target_position: list[float], tolerance=2, timeout=15):
    ...
    def move_to_node(self, target_node_id: int):
        ...
        print(closest_node_id, target_node_id)
    ...
    def move_to_guild_chest(self):
        if self.location != "ISLAND-GUILD":
            return
        self.move_to_node(3)
        self.click(self.mouse_positions["ISLAND-GUILD_chest"])

    def move_to_market(self, destination_market: str):
        self.press("esc")
        if self.location == "": return
        while self.current_location_graph == None:
            self.sleep(0.1)
        if self.location in CONNECTED_LOCATIONS.values() and self.location == CONNECTED_LOCATIONS[destination_market]:
        ...
        if self.location == destination_market:
            ...
            self.move_to_planner()
            self.choose_destination(CONNECTED_LOCATIONS[destination_market])
            while not self.can_move:
                self.sleep(0.1)
            self.move_to_market(destination_market)
    
    def move_to_island(self, destination_island: str):
        self.press("esc")
        self.move_to_planner()
