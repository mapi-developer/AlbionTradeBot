...
class WindowCapture:
    ...
    def __init__(self, window_name: str, base_dir: Optional[str] = "", debugging=False):
    ...
    def get_screenshot(
        ...
        if x_0_crop is None:
            if self.debugging == True:
                self._safe_imwrite(self.BASE_DIR, pil_img)
            return pil_img
        ...
        if self.debugging == True:
            self._safe_imwrite(self.BASE_DIR, pil_img.crop((x0, y0, x1, y1)))
    ...
    def find_template(
        ...
        """
        Locates a template image within the captured window.
        
        :param template_path: Path to the reference image on disk.
        :param threshold: Match confidence (0.0 to 1.0). 0.8 is standard.
        :param search_region: (x0, y0, x1, y1) Optional crop area to search within.
        :return: (x, y) Center coordinates relative to the window top-left, or None.
        """
        # 1. Get the window image (uses your existing method)
        if search_region:
            pil_img = self.get_screenshot(*search_region)
        else:
            pil_img = self.get_screenshot()
        ...
        # 2. Convert PIL -> OpenCV BGR (uses your existing method)
        screen_img = self._pil_to_cv_bgr(pil_img)
